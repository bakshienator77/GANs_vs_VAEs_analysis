...
def build_transforms():
    ...
    # check if the input is already zero to one
...
def get_optimizers_and_schedulers(gen, disc):
    ...
    optim_discriminator = torch.optim.Adam(disc.parameters(), 0.0002, betas=(0.0, 0.9))
    scheduler_discriminator = torch.optim.lr_scheduler.LinearLR(optim_discriminator, start_factor=1.0,
                                                                end_factor=0.0, total_iters=500000)
    optim_generator = torch.optim.Adam(gen.parameters(), 0.0002, betas=(0.0, 0.9))
    scheduler_generator = torch.optim.lr_scheduler.LinearLR(optim_discriminator, start_factor=1.0,
                                                                end_factor=0.0, total_iters=100000)
    return (
        optim_discriminator,
...
def train_model(
    ...
    dataset = Dataset(root="datasets/CUB_200_2011_32", transform=ds_transforms)
            ...
            if iters % log_period == 0 and iters != 0:
                with torch.no_grad():
                    with torch.cuda.amp.autocast():
                        ...
                        generated_samples = (gen(train_batch.shape[0]) + 1.0)/2.0
                        print("Supposed to make sure this lies in [0,1]: ", torch.max(generated_samples), torch.min(generated_samples))
